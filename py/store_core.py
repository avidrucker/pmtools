...
def parse_store_args(argv):
    """Pure CLI arg parser shared by the error + velocity wrappers (their argv
    shape is identical: `<cmd> [json] [--db-path P] [--csv P | --no-csv]`). The
    wrappers previously each inlined this; promoting it here removes that
    duplication and makes it gradeable against the SAME fixtures the JS port
    grades (#46). An unknown --flag raises ValueError (the impure wrapper catches
    it and dies, exit 2). `--db-path`/`--csv` as the last token yield None.
    """
...
def resolve_csv(args, store_cfg):
    """Pure: the CSV mirror path to export to, or None. `--no-csv` wins; then an
